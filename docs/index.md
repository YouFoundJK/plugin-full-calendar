...
If you are new here or is looking for workflow guide head to [User Docs](user/guides/index.md) / use the Search bar above (or press `S`), while if you are a veteran who wants to contribute or is just crazy to find joy in reading the core architecture jump to [Architecture Docs](architecture/system/index.md).

The latest version 0.12.7 introduces **[Local ICS Support](user/calendars/ics.md)**, a dramatically improved **Staged Loading Architecture** for lightning-fast startup, and completely rewritten **[Timezone & DST Hardening](user/events/timezones.md)** to ensure accurate events everywhere.
