...
export class WorkspaceModal extends Modal {
  ...
  onOpen() {
    ...
    // Modal title
    contentEl.createEl('h2', { text: this.isNew ? t('modals.workspace.title.create') : t('modals.workspace.title.edit') });
  ...
  private renderCategoryFilterSection(containerEl: HTMLElement) {
    ...
    new Setting(section)
      ...
      .addDropdown(dropdown => {
        dropdown.addOption('show-only', t('modals.workspace.fields.categoryFilterMode.options.showOnly'));
  ...
  private renderAppearanceSection(containerEl: HTMLElement) {
    ...
    if (this.plugin.settings.enableAdvancedCategorization) {
      new Setting(section)
        ...
        .addDropdown(dropdown => {
          dropdown.addOption('', t('modals.workspace.fields.timelineCategories.options.useDefault'));
          dropdown.addOption('true', t('modals.workspace.fields.timelineCategories.options.expanded'));
          dropdown.addOption('false', t('modals.workspace.fields.timelineCategories.options.collapsed'));
  ...
  private renderButtons(containerEl: HTMLElement) {
    ...
    buttonContainer.createEl(
      'button',
      { text: this.isNew ? t('modals.workspace.buttons.create') : t('modals.workspace.buttons.save'), cls: 'mod-cta' },
      button => {
        button.addEventListener('click', () => {
