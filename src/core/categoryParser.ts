 ...
 * @returns An object containing the parsed `category` (or null) and `title`.
 */
export function parseTitle(fullTitle: string): { category: string | null; title: string } {
  ...
  if (delimiterIndex === -1) {
    ...
    return { category: null, title: fullTitle };
  }
 ...
 *
 * @param category The category string, or null if no category.
 * @param title The clean event title.
 * @returns The reconstructed full title string.
 */
export function constructTitle(category: string | null, title: string): string {
