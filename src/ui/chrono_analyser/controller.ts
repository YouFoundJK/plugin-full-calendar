...
export class AnalysisController {
    ...
    private async parseFile(file: TFile): Promise<TimeRecord> {
        try {
            ...
            if (dateStr) {
                ...
                if (
                    metaDateVal instanceof Date &&
                    !isNaN(metaDateVal.getTime())
                ) {
                    recordDate = new Date(
                        Date.UTC(
                            metaDateVal.getFullYear(),
                            metaDateVal.getMonth(),
                            metaDateVal.getDate(),
                        ),
                    );
                } else {
                    const metaDateStr = String(metaDateVal);
                    const datePartsMatch = metaDateStr.match(
                        /^(\d{4})-(\d{2})-(\d{2})/,
                    );
                    if (datePartsMatch) {
            ...
            if (recordDate && isNaN(recordDate.getTime()))
                throw new Error(
                    `Invalid date parsed: ${dateStr || metadata.date}`,
                );

            const finalProject = projectFromFile
                ? projectFromFile.trim()
                : "Unknown Project";
            let baseSubproject = "none",
                fullSubproject = "none";
            if (subprojectRaw) {
                subprojectRaw = subprojectRaw.trim();
                const subprojectSerialMatch = subprojectRaw.match(
                    /^(.*?)\s+([IVXLCDM\d]+)$/,
                );
    ...
    private setupEventListeners = () => {
        ...
        // Date Picker
        const datePickerEl =
            this.rootEl.querySelector<HTMLInputElement>("#dateRangePicker");
        if (datePickerEl) {
        ...
        this.rootEl
            ...
            ?.addEventListener(
                "click",
                () => new Notice("Cache clearing will be implemented later."),
            );

        // Other UI elements
        this.rootEl
            .querySelector("#clearDatesBtn")
            ?.addEventListener("click", this.clearDateFilters);
        this.rootEl
        ...
        this.rootEl
            ...
            ?.addEventListener("change", this.updateAnalysis);
        this.rootEl
            .querySelector("#timeSeriesTypeSelect")
        ...
        this.rootEl
            ...
            ?.addEventListener("click", this.hideDetailPopup);
        this.rootEl
            .querySelector("#detailOverlay")
            ?.addEventListener("click", this.hideDetailPopup);
    };
    ...
    private handleAnalysisTypeChange = () => {
        const analysisType = this.rootEl.querySelector<HTMLSelectElement>(
            "#analysisTypeSelect",
        )?.value;
        ...
        if (analysisType === "sunburst") {
            ...
            this.rootEl
                .querySelector("#pieCategoryFilterContainer")
                ?.classList.remove("hidden-controls");
        } else if (analysisType === "pie") {
            this.rootEl
                .querySelector("#pieBreakdownLevelContainer")
                ?.classList.remove("hidden-controls");
            this.rootEl
                .querySelector("#pieCategoryFilterContainer")
                ?.classList.remove("hidden-controls");
            ...
            this.rootEl
                ...
                ?.classList.remove("hidden-controls");
            this.rootEl
                .querySelector("#timeSeriesTypeContainer")
                ?.classList.remove("hidden-controls");
            this.handleTimeSeriesTypeVis();
    ...
    private handleTimeSeriesTypeVis = () => {
        const timeSeriesType = this.rootEl.querySelector<HTMLSelectElement>(
            "#timeSeriesTypeSelect",
        )?.value;
    ...
    private saveUIState = () => {
        const getElValue = (id: string) =>
            this.rootEl.querySelector<HTMLInputElement | HTMLSelectElement>(
                `#${id}`,
            )?.value;
        const state: any = {
            ...
            timeSeriesStackingLevel: getElValue(
        ...
        if (
            ...
            this.flatpickrInstance.selectedDates.length === 2
        ) {
            state.startDate = this._getISODate(
                this.flatpickrInstance.selectedDates[0],
            );
            state.endDate = this._getISODate(
    ...
    private updateAnalysis = () => {
        ...
        setTimeout(() => {
            const statsGrid =
                this.rootEl.querySelector<HTMLElement>("#statsGrid");
            const mainChartContainer = this.rootEl.querySelector<HTMLElement>(
                "#mainChartContainer",
            );
            const errorLogContainer =
                this.rootEl.querySelector<HTMLElement>("#errorLogContainer");
            const mainChartEl =
                this.rootEl.querySelector<HTMLElement>("#mainChart");
            const legendEl =
                this.rootEl.querySelector<HTMLElement>("#customLegend");
            ...
            const hideUIElements = () => {
                ...
                const statEl = this.rootEl.querySelector(
            ...
            if (
                this.filteredRecordsForCharts.length === 0 &&
                this.records.length > 0
            ) {
            ...
            (
                this.rootEl.querySelector("#totalHours") as HTMLElement
            ).textContent = filteredDataResults.totalHours.toFixed(2);
            (
                this.rootEl.querySelector("#totalFiles") as HTMLElement
            ).textContent = String(filteredDataResults.fileCount);

            const analysisTypeEl = this.rootEl.querySelector<HTMLSelectElement>(
                "#analysisTypeSelect",
            );
            ...
            if (analysisType === "sunburst") {
                ...
                if (analysisType === "time-series") {
                    ...
                    const pieLevel =
                        this.rootEl.querySelector<HTMLSelectElement>(
                            "#levelSelect_pie",
                        )?.value || "hierarchy";
                    const piePattern =
                        this.rootEl.querySelector<HTMLInputElement>(
                            "#patternInput",
                        )?.value || "";
                    ...
                    if (
                        ...
                        this.renderPieChartDisplay(
                            this.currentPieAggregatedData.hours,
                        );
                    } else {
    ...
    private calculateDuration(
        ...
        const parseTime = (timeStr: any) => {
            if (timeStr == null)
                throw new Error("Invalid time: null or undefined.");
            if (typeof timeStr === "number") {
            ...
            throw new Error(
                `Invalid time format: ${sTimeStr}. Use HH:MM or decimal hours.`,
            );
        };
        try {
    ...
    private calculateRecurringInstancesInDateRange(
        ...
        const effectiveStart = new Date(
            Math.max(
                ...
                filterStartDate?.getTime() || recurrenceStart.getTime(),
            ),
        );
        const effectiveEnd = new Date(
            Math.min(
                recurrenceEnd.getTime(),
                filterEndDate?.getTime() || recurrenceEnd.getTime(),
            ),
        );
    ...
    private getFilteredRecords(): {
        ...
        if (
            ...
            startDateStr =
                this._getISODate(this.flatpickrInstance.selectedDates[0]) || "";
            endDateStr =
                this._getISODate(this.flatpickrInstance.selectedDates[1]) || "";
        }

        let filterStartDate: Date | null = startDateStr
            ? new Date(startDateStr)
            : null;
        let filterEndDate: Date | null = endDateStr
            ? new Date(endDateStr)
            : null;
        ...
        for (const record of this.records) {
            ...
            if (
                hierarchyFilter &&
                record.hierarchy &&
                record.hierarchy.toLowerCase() !== hierarchyFilter
            )
                continue;
            if (
                projectFilter &&
                record.project &&
                record.project.toLowerCase() !== projectFilter
            )
                continue;
            ...
            if (record.metadata?.type === "recurring") {
                if (
                    !record.metadata.startRecur ||
                    !record.metadata.daysOfWeek ||
                    record.duration === 0
                ) {
                    effectiveDuration = 0;
                } else {
                    const numInstances =
                        this.calculateRecurringInstancesInDateRange(
                            record.metadata,
                            filterStartDate,
                            filterEndDate,
                        );
    ...
    private aggregateForSunburst(
        filteredRecords: TimeRecord[],
        level: string,
    ): SunburstData {
        console.log(
            `[Chrono Analyser] Aggregating for Sunburst level: "${level}"`,
        );
        ...
        for (const record of filteredRecords) {
            ...
            const innerVal = String(
                record[innerField] || `(No ${innerField})`,
            ).trim();
            const outerVal = String(
                record[outerField] || `(No ${outerField})`,
            ).trim();
        ...
        for (const [parent, total] of parentTotals.entries()) {
            ...
            const parentRecords = filteredRecords.filter(
                (r) =>
                    String(r[innerField] || `(No ${innerField})`).trim() ===
                    parent,
            );
    ...
    private aggregateForPieChart(
        ...
        for (const record of filteredRecords) {
            ...
            hours.set(
                key,
                (hours.get(key) || 0) +
                    (record._effectiveDurationInPeriod as number),
            );
    ...
    private renderPieChartDisplay(hoursData: Map<string, number>) {
        ...
        const levelSelect =
            this.rootEl.querySelector<HTMLSelectElement>("#levelSelect_pie");
        ...
        plotlyChart.on("plotly_click", (eventData: any) => {
            if (eventData.points && eventData.points.length > 0) {
                ...
                if (
                    this.currentPieAggregatedData?.recordsByCategory.has(
                        categoryName,
                    )
                ) {
                    this.showDetailPopup(
                        categoryName,
                        this.currentPieAggregatedData.recordsByCategory.get(
                            categoryName,
                        )!,
        ...
        plotlyChart.on("plotly_sunburstclick", (eventData: any) => {
            if (eventData.points && eventData.points.length > 0) {
                ...
                if (point.id && sunburstData.recordsByLabel.has(point.id)) {
                    this.showDetailPopup(
                        point.label,
                        sunburstData.recordsByLabel.get(point.id)!,
                        { type: "sunburst", value: point.value },
                    );
                }
            }
        });
    ...
    private renderTimeSeriesChart() {
        ...
        const chartTypeEl = this.rootEl.querySelector<HTMLSelectElement>(
            "#timeSeriesTypeSelect",
        );
        ...
        this.filteredRecordsForCharts.forEach((record) => {
            if (record.metadata?.type === "recurring") {
                const { startRecur, endRecur, daysOfWeek, duration } =
                    record.metadata;
                ...
                while (iterDate <= maxDate) {
                    if (actualDays.includes(iterDate.getUTCDay())) {
                        ...
                        if (granularity === "daily")
                            periodKey = this._getISODate(iterDate);
                        else if (granularity === "weekly")
                            periodKey = this._getISODate(
                                this._getWeekStartDate(iterDate),
                            );
                        else
                            periodKey = this._getISODate(
                                this._getMonthStartDate(iterDate),
                            );
                        ...
                        if (periodKey) {
                            ...
                            if (chartType === "stackedArea") {
                                const category = String(
                                    record[stackingLevel] ||
                                        `(No ${stackingLevel})`,
                                );
                                periodData.categories[category] =
                                    (periodData.categories[category] || 0) +
                                    duration;
                            }
                        }
                    }
                ...
                if (granularity === "daily")
                    periodKey = this._getISODate(record.date);
                else if (granularity === "weekly")
                    periodKey = this._getISODate(
                        this._getWeekStartDate(record.date),
                    );
                else
                    periodKey = this._getISODate(
                        this._getMonthStartDate(record.date),
                    );
                ...
                if (chartType === "stackedArea") {
                    const category = String(
                        record[stackingLevel] || `(No ${stackingLevel})`,
                    );
                    periodData.categories[category] =
                        (periodData.categories[category] || 0) +
                        (record._effectiveDurationInPeriod || 0);
                }
            }
        });
        ...
        if (chartType === "line") {
            traces.push({
                ...
                y: sortedPeriods.map((p) =>
                    dataByPeriod.get(p)!.total.toFixed(2),
                ),
                type: "scatter",
                mode: "lines+markers",
                name: "Total Hours",
            ...
            sortedPeriods.forEach((p) =>
                Object.keys(dataByPeriod.get(p)!.categories).forEach((cat) =>
            ...
            Array.from(allCategories)
                ...
                .forEach((category) => {
                    traces.push({
                        ...
                        y: sortedPeriods.map((p) =>
                            (
                                dataByPeriod.get(p)!.categories[category] || 0
                            ).toFixed(2),
                        ),
                        type: "scatter",
                        mode: "lines",
    ...
    private renderActivityPatternChart() {
        ...
        const analysisTypeName =
            patternTypeEl.selectedOptions[0]?.text || "Activity Pattern";
        ...
        if (patternType === "dayOfWeek") {
            ...
            records.forEach((record) => {
                if (record.metadata?.type === "recurring") {
                    ...
                    // --- THIS IS THE FIX ---
                    const startRecurStr = this._getISODate(
                        new Date(startRecur),
                    );
                    ...
                    let recEnd = new Date(Date.UTC(9999, 0, 1));
                    if (endRecur) {
                        const endRecurStr = this._getISODate(
                            new Date(endRecur),
                        );
                    ...
                    let iterDate = new Date(
                        Math.max(
                            recStart.getTime(),
                            filterStartDate?.getTime() || recStart.getTime(),
                        ),
                    );
                    let maxDate = new Date(
                        Math.min(
                            recEnd.getTime(),
                            filterEndDate?.getTime() || recEnd.getTime(),
                        ),
                    );
            ...
            records.forEach((record) => {
                ...
                if (record.metadata?.type === "recurring") {
                    ...
                    if (record.date && !isNaN(record.date.getTime())) {
                        const dayIndex = record.date.getUTCDay();
                        heatmapData[dayIndex][startHour] +=
                            record._effectiveDurationInPeriod || 0;
                    }
                }
            });
            data = [
                {
                    z: heatmapData.map((row) =>
                        row.map((val) => (val > 0 ? val.toFixed(2) : null)),
                    ),
                    x: hourLabels,
                    y: daysOfWeekLabels,
                    type: "heatmap",
        ...
        if (
            !data.length ||
            (plotType === "bar" &&
                (data[0] as any).y.every(
                    (val: string) => parseFloat(val) === 0,
                )) ||
            (plotType === "heatmap" &&
                (data[0] as any).z
                    .flat()
                    .every((val: string | null) => val === null))
        ) {
        ...
        plotlyChart.on("plotly_click", (eventData: any) => {
            ...
            if (plotType === "bar") {
                ...
                if (patternType === "dayOfWeek") {
                    const dayIndexClicked =
                        daysOfWeekLabels.indexOf(categoryClicked);
                ...
                if (patternType === "hourOfDay") {
                    ...
                    recordsForPopup = records.filter(
                        (r) =>
                            ...
                            this._getHourFromTimeStr(r.metadata.startTime) ===
                                hourClicked,
                    );
                }
    ...
    private renderErrorLog() {
        ...
        const errorLogSummary =
            this.rootEl.querySelector<HTMLElement>("#errorLogSummary");
        const errorLogEntries =
            this.rootEl.querySelector<HTMLElement>("#errorLogEntries");
        ...
        if (this.processingErrors.length === 0) {
            errorLogSummary.textContent =
                "No processing issues found for the last selected folder.";

            // --- THIS IS THE FIXED LINE ---
            // The reference to `this.cache.size` has been removed.
            errorLogContainer.style.display =
                this.records.length > 0 || this.processingErrors.length > 0
                    ? "block"
                    : "none";
            // -----------------------------
    ...
    private showDetailPopup(
        ...
        const popupTitleEl =
            this.rootEl.querySelector<HTMLElement>("#popupTitle");
        const popupSummaryStatsEl =
            this.rootEl.querySelector<HTMLElement>("#popupSummaryStats");
        const tableBody =
            this.rootEl.querySelector<HTMLTableSectionElement>(
                "#popupTableBody",
            );
        const detailOverlay =
            this.rootEl.querySelector<HTMLElement>("#detailOverlay");
        const detailPopup =
            this.rootEl.querySelector<HTMLElement>("#detailPopup");
        if (
            !popupTitleEl ||
            !popupSummaryStatsEl ||
            !tableBody ||
        ...
        const displayTotalHours =
            context.value ??
            recordsList.reduce(
                (sum: number, r: TimeRecord) =>
                    sum + (r._effectiveDurationInPeriod || 0),
                0,
            );
        ...
        recordsList.forEach((record) => {
            ...
            dateCell.textContent = record.date
                ? this._getISODate(record.date)
                : "Recurring";
            row.insertCell().textContent = (
                record._effectiveDurationInPeriod || record.duration
            ).toFixed(2);
    ...
    private hideDetailPopup = () => {
        this.rootEl
            .querySelector<HTMLElement>("#detailOverlay")
            ?.classList.remove("visible");
        this.rootEl
            .querySelector<HTMLElement>("#detailPopup")
            ?.classList.remove("visible");
    };
    ...
    private clearAllFilters = () => {
        ...
        const hierarchyInput = this.rootEl.querySelector<HTMLInputElement>(
            "#hierarchyFilterInput",
        );
        if (hierarchyInput) hierarchyInput.value = "";

        const projectInput = this.rootEl.querySelector<HTMLInputElement>(
            "#projectFilterInput",
        );
    ...
    private updateActiveSuggestion(suggestions: HTMLElement[], index: number) {
        suggestions.forEach((suggestion, idx) =>
            suggestion.classList.toggle("active", idx === index),
        );
    }

    private populateFilterDataSources() {
        this.allHierarchies = [
            ...new Set(this.records.map((r) => r.hierarchy).filter(Boolean)),
        ].sort();
        this.allProjects = [
            ...new Set(this.records.map((r) => r.project).filter(Boolean)),
        ].sort();
    ...
    private setupAutocomplete(
        ...
        const suggestionsContainer = this.rootEl.querySelector<HTMLElement>(
            `#${suggestionsId}`,
        );
        ...
        input.addEventListener("keydown", (e: KeyboardEvent) => {
            let currentSuggestions = Array.from(
