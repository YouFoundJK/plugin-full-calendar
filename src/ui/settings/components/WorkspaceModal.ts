    ...
    calendars.forEach(calendar => {
      ...
      if (!displayName || displayName.trim() === '') {
        switch (calendar.type) {
          case 'local':
            displayName = `Local: ${calendar.config.directory}`;
            break;
          case 'dailynote':
            displayName = `Daily Notes: ${calendar.config.heading}`;
            break;
          case 'ical':
            try {
              displayName = `ICS: ${new URL(calendar.config.url).hostname}`;
            } catch (_) {
              displayName = 'ICS Calendar';
            }
            break;
          case 'caldav':
            displayName = `CalDAV: ${calendar.config.name}`;
            break;
          case 'google':
            displayName = `Google: ${calendar.config.name}`;
            break;
          default:
