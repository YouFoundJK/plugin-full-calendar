  ...
  return new Setting(containerEl)
    ...
    .addExtraButton(button => {
      ...
      button.onClick(async () => {
        ...
        if (sourceType === 'local') {
          const provider = plugin.providerRegistry.getProvider('local');
          if (!provider) {
            new Notice('Full Note provider is not registered.');
            return;
          }
          ...
          let modal = new ReactModal(plugin.app, async () => {
            ...
            const usedDirectories = plugin.settings.calendarSources
            ...
            return createElement(ConfigComponent, {
              ...
              onSave: finalConfig => {
                // Construct the full CalendarInfo object and submit it.
                const finalSource = {
                  ...newConfigSource, // This already has the color
                  config: finalConfig,
                  type: 'local', // Still need legacy type for now
                  name: (finalConfig as any).directory, // Default name to directory
                  id: '' // Will be generated by the logic that calls submitCallback
                };
                // Cast to unknown first to satisfy strict type checking.
