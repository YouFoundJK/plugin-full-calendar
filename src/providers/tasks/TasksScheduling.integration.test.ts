...
describe('Task Scheduling Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockApp = {
      read: jest.fn(),
      rewrite: jest.fn().mockImplementation(async (file: any, modifyFn: (content: string) => string) => {
  ...
  describe('scheduleTask method', () => {
    it('should call _invalidateCache after successfully scheduling a task', async () => {
      ...
      mockApp.rewrite.mockImplementation(async (file: any, modifyFn: (content: string) => string) => {
    ...
    it('should handle errors gracefully and not call _invalidateCache on failure', async () => {
      ...
      await expect(provider.scheduleTask('invalid-task-id', testDate)).rejects.toThrow('Failed to schedule task');
